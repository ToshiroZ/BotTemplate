namespace Discord.Addons.Interactive
{
    ...
    public class InteractiveService : IDisposable
    {
        ...
        public async Task<IUserMessage> OkAsync(SocketCommandContext context, string content, bool isTTS = false, TimeSpan? timeout = null, RequestOptions options = null)
        {
            var embed = new EmbedBuilder { Title = "Ok", Description = content, Color = Color.Magenta }.Build();
            var message = await context.Channel.SendMessageAsync(null, isTTS, embed, options).ConfigureAwait(false);
            if (timeout.HasValue)
            {
                _ = Task.Delay(timeout.Value)
